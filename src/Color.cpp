...
target::Color::Color(const Color & color){
	r = color.r;
	g = color.g;
	b = color.b;
}

target::Color::Color(const std::string & color, std::map<std::string, Color> & pallete){
...
target::Color::channel& target::Color::operator[](int idx){
	switch(idx){
		case 0:
			return r;
		case 1:
...
const target::Color::channel& target::Color::operator[](int idx) const{
	switch(idx){
		case 0:
			return r;
		case 1:
...
bool target::Color::operator==(const target::Color & color) const{
