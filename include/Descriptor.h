...
namespace target{
	class Descriptor{
		private:
			...
			static std::vector< std::shared_ptr<target::Triangle> > processMeshObject(const std::string & filename, const std::string & name, const bool & clk = true, const bool & bfc = true);
