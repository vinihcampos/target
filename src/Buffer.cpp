...
target::Buffer::Buffer(size_t width_, size_t height_, const Color & tl_, const Color & bl_, const Color & br_, const Color & tr_) 
	...
	buffer = std::make_unique<component_t[]>(width * height * 3);
	std::memset(buffer.get(), 0, width * height * 3 * sizeof(component_t));
}
...
void target::Buffer::fill(const Color & color, const Point2d p, const Color & borderColor){
...
target::Color target::Buffer::get(const target::Point2d & p) const{
...
void target::Buffer::pixel(const Point2d & p, const Color & color){
...
void target::Buffer::pixels(const Point2d ps[], const size_t & p_size, const Color & color){
